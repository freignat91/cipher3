...
One way to get data very difficult to decrypt is to use a real random key as long as the data to encrypt and never reuse it two times.
...
the only way is the brute force algorithm trying each possible key values, but with a key 3 * 16384 * 8 bytes, which is the smallest key size used in this project the time to do that is too huge. 
...
- prerequisite: go installed and $GOPATH set
